...
export default async function CardListView({
  ...
  const bySport = filteredByName.reduce<Record<string, Entity[]>>(
    (acc, ent) => {
            ...
            <div key={sportName} className="mb-6">
              ...
              <div className="grid xl:grid-cols-3 sm:max-xl:grid-cols-2 gap-2">
                {entities.map((entity) => {
                  ...
                  const teamName =
                    entity.teams?.[0]?.name || "Individual player";
                  const isPlayer =
                    entity.type.id === EntityType.IndividualPlayer ||
                    entity.type.id === EntityType.TeamPlayer;
